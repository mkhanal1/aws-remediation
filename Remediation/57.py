...
Usage: This module will add encryption in transit via Bucket Policy for the affected buckets."
...
def action(boto_session,query,resourceId,region_name,accountId):
    ...
    responsedata = json.loads(response['Policy'])
    for i in responsedata['Statement']:
        i['Condition'] = {"Bool":{"aws:SecureTransport":"false"}}
    print (responsedata)
    try:
        response = s3.put_bucket_policy(
            Bucket=resourceId,
            ConfirmRemoveSelfBucketAccess=False,
            Policy=json.dumps(responsedata)
        )
        status_code = response['ResponseMetadata']['HTTPStatusCode']
        if status_code > 400:
            debug_module.append("Bucket policy could not be added for encryption on transit!")
            debug_module.append(str(response))
        else:
            debug_module.append("Bucket policy added successfully for encryption on transit!")
